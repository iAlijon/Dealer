...
if(!function_exists('infoByDomDocment')) {

    function infoByDomDocment($content)
    {
        $dom = new \DomDocument();
        ...
        if (count($images) > 0) {
            foreach ($images as $k => $img) {
                ...
                if ($isData) {
                    ...
                    $directory =  "/uploads/news_info/images/" . date('Y') . "/" . date('m') . "/" . date('d')."/";
        ...
        if (count($links) > 0) {
            foreach ($links as $k => $link) {
                ...
                if(preg_match('/\bblob:\b/', $dataFile)) {
                    ...
                    $directory =  "/uploads/news_info/files/" . date('Y') . "/" . date('m') . "/" . date('d')."/";
                    $path = public_path() . $directory;
                    if(!is_dir($path)) {
                        mkdir($path, 0755, true);
                    }
